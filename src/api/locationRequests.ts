import { GeoCoordinatesResponse, LocationNameResponse, TimeZoneResponse } from '../types/responseInterfaces'
...
export const getGeoCoordinatesByCityName = async (cityName: string): Promise<GeoCoordinatesResponse> => {
    ...
    try {
        ...
        const locationData = await response.json()
        return { data: locationData[0] as GeoCoordinatesResponse['data'], error: null }
    } catch (error) {
...
export const getTimeZoneByCoordinates = async (latitude: number, longitude: number): Promise<TimeZoneResponse> => {
    ...
    try {
        ...
        return { offset: null, error: 'Error fetching time zone' }
    }
}